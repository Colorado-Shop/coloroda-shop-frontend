...
export class CategoryComponent implements OnInit {
  ...
  constructor(
    ...
    this.route.params.subscribe((params) => {
      ...
      if (this.id == "strain") {
        this.description = `Each cannabis strain has a different concentration of the cannabinoids tetrahydrocannabinol (THC), cannabidiol (CBD), as well as other compounds. Cultivators and farmers have long been breeding different varieties with certain characteristics to create hybrids and specific strains.`
      } else if (this.id == "concentrate") {
        this.description = `Cannabis concentrate, also called marijuana concentrate, marijuana extract, or cannabis extract, is a tetrahydrocannabinol (THC) and/or cannabidiol (CBD) concentrated mass. Cannabis concentrates contain high THC levels that range from 40% to over 90%,[1][2] stronger in THC content than high-grade marijuana, which normally measures around 20% THC levels.`
      } else if (this.id == "seed") {
        this.description = `Our comprehensive selection of feminized cannabis seeds has been expertly bred to produce only female cannabis plants. Female cannabis plants are guaranteed to provide cannabinoid-rich flowers for growers to harvest.`
      } else if (this.id == "vape_pen") {
        this.description = `
